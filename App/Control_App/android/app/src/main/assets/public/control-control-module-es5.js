...
var ControlPage = /** @class */ (function () {
    function ControlPage(connectionServices, router, http) {
        //   this.id = setInterval(() => {
    ...
    ControlPage.prototype.onPhoto = function () {
        console.log("Photo");
        this.photoImg = 'http://' + this.connectionServices.getIP() + ':' +
            this.connectionServices.getPort() + '/image?' + new Date().getTime();
    };
    ControlPage.prototype.onVideo = function () {
        console.log("Video");
        window.location.href = 'http://' + this.connectionServices.getIP() + ':5000' + '/';
    };
    ControlPage.prototype.onAlarm = function () {
        var _this = this;
        console.log("Alarm");
        var params = new _angular_common_http__WEBPACK_IMPORTED_MODULE_3__["HttpParams"]().set('alarm', '');
        this.http.get('http://' + this.connectionServices.getIP() + ':' +
            this.connectionServices.getPort() + '/', { params: params }).subscribe(function (data) {
            console.log(data);
            _this.dataObject = data;
            _this.iconAlarm = _this.dataObject['state'] ? 'notifications-off' : 'notifications';
        });
    };
    ControlPage.prototype.onLock = function () {
        var _this = this;
        console.log("Lock");
        var params = new _angular_common_http__WEBPACK_IMPORTED_MODULE_3__["HttpParams"]().set('lock', '');
        this.http.get('http://' + this.connectionServices.getIP() + ':' +
            this.connectionServices.getPort() + '/', { params: params }).subscribe(function (data) {
            console.log(data);
            _this.dataObject = data;
            _this.iconLock = _this.dataObject['state'] ? 'lock' : 'unlock';
        });
    };
    ControlPage.prototype.onUpdateColor = function (value) {
        console.log("Lights turned to " + value);
        var params = new _angular_common_http__WEBPACK_IMPORTED_MODULE_3__["HttpParams"]().set('light', value);
        this.http.get('http://' + this.connectionServices.getIP() + ':' +
            this.connectionServices.getPort() + '/', { params: params }).subscribe(function (data) {
            console.log(data);
        });
    };
    ControlPage.ctorParameters = function () { return [
